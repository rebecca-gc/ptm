...
def swiss_prot():
    filepath = os.path.join(METHYL_DIR, 'swissProt.fasta')

    url = 'https://rest.uniprot.org/uniprotkb/stream?format=fasta&query=%28%28organism_id%3A9606%29+AND+%28reviewed%3Atrue%29+AND+%28keyword%3AKW-0488%29%29'
...
def ncbi():
    filepath = os.path.join(METHYL_DIR, 'ncbi.fasta')

    Entrez.email = 'rebeg02@zedat.fu-berlin.de'
    query = '"Homo sapiens"[Organism] AND Methylation[All Fields]'
...
def db_ptm():
    zippath = os.path.join(METHYL_DIR, 'dbPTM.gz')
    txtpath = os.path.join(METHYL_DIR, 'dbPTM.txt')
    filepath = os.path.join(METHYL_DIR, 'dbPTM.fasta')

    url = 'https://biomics.lab.nycu.edu.tw/dbPTM/download/experiment/Methylation.gz'
...
def ptmd():
    zippath = os.path.join(METHYL_DIR, 'ptmd.zip')
    txtpath = os.path.join(METHYL_DIR, 'Methylation')
    filepath = os.path.join(METHYL_DIR, 'ptmd.fasta')

    url = 'https://ptmd.biocuckoo.cn/Download/Methylation.zip'
    ...
    shutil.unpack_archive(zippath, METHYL_DIR, 'zip')
...
def ptm_code2():
    zippath = os.path.join(METHYL_DIR, 'PTMcode2.zip')
    txtpath = os.path.join(METHYL_DIR, 'PTMcode2_associations_within_proteins.txt')
    filepath = os.path.join(METHYL_DIR, 'PTMcode2.fasta')
...
def qptm():
    filepath = os.path.join(METHYL_DIR, 'qPTM.fasta')
    ...
    with open(txtpath, 'r') as f:
        for line in f:
            ...
            if words[0] == 'Human' and words[5] == 'Methylation':

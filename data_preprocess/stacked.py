...
def aggregate_ptm_data(tsv_dir):
    ...
    entries_per_year_ptm = (
        df_all.groupby(["year", "ptm"]).size().unstack(fill_value=0)
    )
    cumulative_counts = entries_per_year_ptm.cumsum()
    cumulative_percentages = (
        cumulative_counts.div(cumulative_counts.sum(axis=1), axis=0) * 100
    )
...
def plot_cumulative_counts(cumulative_counts, output_dir):
    ...
    cumulative_counts.plot(
        kind="bar", stacked=True, figsize=(10, 6), color=custom_colors, zorder=2
    )
...
def plot_cumulative_percentages(cumulative_percentages, output_dir):
    ...
    cumulative_percentages.plot(
        kind="bar", stacked=True, figsize=(10, 6), color=custom_colors
    )
